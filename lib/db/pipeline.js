    ...
    include: {
      ...
      account: {
        select: {
          id:    true,
          name:  true,
          // Latest deal so we can surface totalMRR on ClosedWon cards
...
function enrichLead(lead) {
  ...
  const dealMRR     = latestDeal ? Number(latestDeal.totalMRR) : null
  const dealCountry = latestDeal?.countryCode ?? null   // e.g. "KSA", "Egypt"

  return {
    ...lead,
    estimatedValue: lead.estimatedValue ? Number(lead.estimatedValue) : null,
    daysOld: diffDays,
    dealMRR,        // actual MRR from the closed deal (null if no deal)
    dealCountry,    // country code for FX conversion of dealMRR
  }
}
