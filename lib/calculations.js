  ...
  return {
    ...
    account: contract.account
      ? {
          ...
          // usdRate=1 means contractValue is already stored in USD (bulk-imported data)
          currency: (!usdRate || usdRate === 1) ? 'USD' : (contract.account.country?.currency ?? 'USD'),
        }
      : contract.account,
  }
}
...
export function accountStatus(contracts) {
  ...
  const hasActive = contracts.some(
    (c) => !c.cancellationDate && today <= new Date(c.endDate)
  )
  if (hasActive) return 'Active'
  // Distinguish intentional churn (cancellationDate set) from natural expiry
  const hasCancelled = contracts.some((c) => c.cancellationDate != null)
  return hasCancelled ? 'Churned' : 'Expired'
}
