...
async function getAllContracts(filters = {}) {
  const where = {}
  const accountWhere = {}
  if (filters.countries?.length)         accountWhere.country    = { code: { in: filters.countries } }
  else if (filters.country)              accountWhere.country    = { code: filters.country }
  if (filters.leadSources?.length > 0)   accountWhere.leadSource = { in: filters.leadSources }
  if (filters.accountManagerIds?.length) accountWhere.onboarding = { accountManagerId: { in: filters.accountManagerIds } }
  else if (filters.accountManagerId)     accountWhere.onboarding = { accountManagerId: filters.accountManagerId }
...
async function getAllAccounts(filters = {}) {
  const where = {}
  if (filters.countries?.length)         where.country    = { code: { in: filters.countries } }
  else if (filters.country)              where.country    = { code: filters.country }
  if (filters.leadSources?.length > 0)   where.leadSource = { in: filters.leadSources }
  if (filters.accountManagerIds?.length) where.onboarding = { accountManagerId: { in: filters.accountManagerIds } }
  else if (filters.accountManagerId)     where.onboarding = { accountManagerId: filters.accountManagerId }
...
export async function getChurnedAccounts(filters = {}) {
  ...
  for (const acc of accounts) {
    ...
    // Type filter (supports both single string and array)
    const types = filters.types?.length ? filters.types : (filters.type ? [filters.type] : [])
    if (types.length && !types.some((t) => (t === 'churned' && status === 'Churned') || (t === 'expired' && status === 'Expired'))) continue
    ...
    // Currency: if all contracts have usdRate = 1 (or null), values are already
    // in USD (bulk-imported data). Otherwise use the account's local currency.
    const allUSD = acc.contracts.every((c) => !c.usdRate || Number(c.usdRate) === 1)
    const currency = allUSD ? 'USD' : (acc.country?.currency ?? 'USD')

    const onb = acc.onboarding ?? null
    ...
    rows.push({
      ...
      accountManager:   onb?.accountManager?.name ?? null,
      status,
...
export async function getChurnSummary(filters = {}) {
  ...
  for (const acc of accounts) {
    ...
    // For summary date range: compute exit date for lost accounts and filter
    if (!hasActive && acc.contracts.length > 0 && (from || to)) {
      const exitDate = hasCancelled
        ? acc.contracts.filter((c) => c.cancellationDate)
            .reduce((l, c) => { const d = new Date(c.cancellationDate); return d > l ? d : l }, new Date(0))
        : acc.contracts
            .reduce((l, c) => { const d = new Date(c.endDate); return d > l ? d : l }, new Date(0))
      if (from && exitDate < from) continue
      if (to   && exitDate > to)   continue
    }
