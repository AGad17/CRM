...
export async function getDashboardKPIs(filters = {}) {
  ...
  // Mirror the same logic as accountStatus() in lib/calculations.js:
  // an account is Churned when it has contracts but none are currently active
  // (active = no cancellationDate AND endDate >= today). Naturally-expired
  // contracts with no cancellationDate still count as churned.
