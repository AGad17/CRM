...
const VALID_TRANSITIONS = {
  ...
  ClosedWon:  ['Expired', 'Churned'],
  Expired:    ['ClosedWon', 'Churned'],  // Renewed → ClosedWon  |  definitely lost → Churned
  ClosedLost: [],
  Churned:    [],
}
...
export async function createLead(data) {
  const lead = await prisma.lead.create({
    data: {
      ...
      numberOfBranches:  data.numberOfBranches  ? Number(data.numberOfBranches) : null,
...
export async function updateLead(id, data) {
  ...
  const lead = await prisma.lead.update({
    ...
    data: {
      ...
      numberOfBranches:  data.numberOfBranches  != null ? Number(data.numberOfBranches) : null,
...
export async function updateLeadStage(id, newStage, extra = {}) {
  ...
  if (newStage === 'Expired')    updateData.lostReason = null // clear any prior lost reason
...
// ─── Auto-sync: move ClosedWon leads to Expired when contracts have lapsed ────
// Called automatically on every pipeline GET so the Kanban always reflects
// the current contract state without any manual action.
export async function syncExpiredLeads() {
  const now = new Date()

  const leads = await prisma.lead.findMany({
    where: { stage: 'ClosedWon', accountId: { not: null } },
    select: {
      id: true,
      account: {
        select: {
  ...
  const toExpire = leads.filter(({ account }) => {
    ...
    return !hasActive && !hasCancelled // natural lapse → Expired
  })

  if (toExpire.length > 0) {
    await prisma.lead.updateMany({
      where: { id: { in: toExpire.map(l => l.id) } },
      data:  { stage: 'Expired', lostReason: null },
    })
  }

  return { synced: toExpire.length }
}
...
export async function migrateAccountsToLeads(adminUserId) {
  ...
  for (const account of accounts) {
    ...
    const stage  = status === 'Active' ? 'ClosedWon' : status === 'Expired' ? 'Expired' : 'Churned'
