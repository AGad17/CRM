...
export async function getChurnedAccounts(filters = {}) {
  ...
  for (const acc of accounts) {
    ...
    // Branch count: prefer account-level field, fall back to max across contracts
    const contractMaxBranches = acc.contracts.reduce((m, c) => Math.max(m, Number(c.numberOfBranches || 0)), 0)
    const numberOfBranches    = acc.numberOfBranches || contractMaxBranches || null
...
export async function getChurnSummary(filters = {}) {
  ...
  for (const acc of accounts) {
    ...
    const branches = acc.numberOfBranches
      || acc.contracts.reduce((m, c) => Math.max(m, Number(c.numberOfBranches || 0)), 0)
      || 1
    totBr += branches
