...
export async function getAccounts(filters = {}) {
  ...
  // Multi-value support: filters.countries (array) takes precedence over filters.country (string)
  ...
  // Multi-value support: filters.leadSources (array) takes precedence over filters.leadSource (string)
  if (filters.leadSources?.length)     where.leadSource = { in: filters.leadSources }
  else if (filters.leadSource)         where.leadSource = filters.leadSource
...
export async function getAccountById(id) {
  const account = await prisma.account.findUnique({
    ...
    include: {
      contracts: true,
      country: COUNTRY_SELECT,
      brandNames: { select: { id: true, name: true }, orderBy: { name: 'asc' } },
      leads: {
        select: {
          id: true,
          companyName: true,
...
export async function churnAccount(id) {
  ...
  // Also move the ops tracker to Churned (if it exists and isn't already there)
  await prisma.onboardingTracker.updateMany({
    where: { accountId: id, phase: { not: 'Churned' } },
    data:  { phase: 'Churned' },
  })
...
function enrichAccount(account) {
  ...
  const now = new Date()
  const totalMRR = sumMRR(enrichedContracts.filter((c) => !c.cancellationDate && now <= new Date(c.endDate)))
  const contractCount = enrichedContracts.length

  // lastMRR: MRR the account was generating when it was last active
  // For expired accounts (no cancellationDate), use the most recently ended non-cancelled contracts
  // For churned accounts (all cancelled), fall back to all contracts with the latest endDate
  const nonCancelled = enrichedContracts.filter((c) => !c.cancellationDate)
  const pool = nonCancelled.length > 0 ? nonCancelled : enrichedContracts
  const lastMRR = pool.length > 0
    ? (() => {
        const maxMs = Math.max(...pool.map((c) => new Date(c.endDate).getTime()))
        return sumMRR(pool.filter((c) => new Date(c.endDate).getTime() === maxMs))
      })()
    : 0
