...
export async function getInvoiceById(id) {
  return prisma.invoice.findUnique({ where: { id: Number(id) } })
}
...
export async function getARReport() {
  ...
  const allInvoices = await prisma.invoice.findMany({
    select: {
      id: true, invoiceNumber: true, posSystem: true, status: true,
      amountInclVAT: true, eligibleCollectionDate: true, invoiceDate: true,
      accountName: true, deal: { select: { accountId: true } },
    },
  })

  function daysSinceEligible(inv) {
    if (!inv.eligibleCollectionDate) return null
    return Math.floor((today - new Date(inv.eligibleCollectionDate)) / 86400000)
  }
  ...
  // Build aging detail: outstanding invoices only (Pending or Eligible)
  const agingRows = allInvoices
    .filter((i) => i.status !== 'Collected')
    .map((i) => {
      const days = daysSinceEligible(i)
      return {
        id:                    i.id,
        invoiceNumber:         i.invoiceNumber,
        accountName:           i.accountName,
        accountId:             i.deal?.accountId || null,
        posSystem:             i.posSystem,
        status:                i.status,
        amountInclVAT:         Number(i.amountInclVAT),
        eligibleCollectionDate: i.eligibleCollectionDate,
        invoiceDate:           i.invoiceDate,
        daysOverdue:           days !== null && days > 0 ? days : 0,
