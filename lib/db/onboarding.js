...
export async function setPhase(id, phase) {
  ...
  const tracker = await prisma.onboardingTracker.findUnique({ where: { id: Number(id) } })
  if (!tracker) throw new Error('Tracker not found')
  return _doSetPhase(tracker, phase)
}

// Internal: update phase, reset phaseStartedAt, create CSAT record
async function _doSetPhase(tracker, toPhase) {
  const fromPhase = tracker.phase
  const now       = new Date()

  const updated = await prisma.onboardingTracker.update({
    where: { id: tracker.id },
    data:  { phase: toPhase, phaseStartedAt: now },
  })

  // Auto-create CSAT record on every forward transition
  const fromIdx = PHASE_ORDER.indexOf(fromPhase)
  const toIdx   = PHASE_ORDER.indexOf(toPhase)
  if (fromPhase !== toPhase && toIdx > fromIdx) {
