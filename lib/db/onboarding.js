...
const DEFAULT_TASKS_MAP = {
  ...
  // ── Stage 6: Expired ─────────────────────────────────────────────────────────
  // No default tasks — expiry is an external event, not an ops-team milestone.
  // Tasks are managed ad-hoc by the CS team during the renewal pursuit.
  Expired: [],
}
...
export async function getOnboardingTrackers(filters = {}) {
  ...
  const trackers = await prisma.onboardingTracker.findMany({
    ...
    include: {
      account: {
        select: {
          id: true, name: true, countryId: true, leadSource: true,
          country:   { select: { name: true } },
          contracts: { select: { id: true, startDate: true, endDate: true, cancellationDate: true, contractValue: true }, orderBy: { endDate: 'asc' } },
        },
      },
      deal:           { select: { id: true, package: true, posSystem: true, startDate: true, salesChannel: true, totalMRR: true } },
...
function enrichTracker(tracker) {
  ...
  // MRR: prefer the deal's snapshotted totalMRR; fall back to computing from
  // the account's active contracts (for migrated accounts with no deal)
  const dealMRR = tracker.deal?.totalMRR ? Number(tracker.deal.totalMRR) : null
  const now     = new Date()
  const activeContracts = (tracker.account?.contracts ?? []).filter(
    (c) => !c.cancellationDate && c.endDate && new Date(c.endDate) >= now && c.contractValue
  )
  const accountMRR = dealMRR ?? (activeContracts.length > 0 ? sumMRR(activeContracts) : null)
  ...
  return {
    ...
    accountMRR,             // deal MRR ▸ active contracts MRR ▸ null
  }
}
...
export async function advancePhase(id) {
  ...
  // Hard block: all current-phase tasks must be complete before advancing
  _assertTasksComplete(tracker)
...
export async function setPhase(id, phase, assignments = {}) {
  ...
  // Hard block on forward moves.
  // Churned is always allowed as a terminal state (business decision).
  // Expired is also exempt — it reflects an external event (contract lapsed),
  // not ops-team progress, so it can be set regardless of task completion.
  const currentIdx = PHASE_ORDER.indexOf(tracker.phase)
  const newIdx     = PHASE_ORDER.indexOf(phase)
  if (newIdx > currentIdx && phase !== 'Churned' && phase !== 'Expired') {
    _assertTasksComplete(tracker)
  }
...
// Throws if any task in the current phase is not completed
function _assertTasksComplete(tracker) {
  const incomplete = (tracker.tasks ?? []).filter(
    (t) => t.phase === tracker.phase && !t.completed
  )
  if (incomplete.length > 0) {
    throw new Error(
      `Complete all ${incomplete.length} task(s) in ${PHASE_LABELS[tracker.phase] ?? tracker.phase} before advancing`
    )
  }
}
...
async function _doSetPhase(tracker, toPhase, assignments = {}) {
  ...
  // Churn capture — only written when transitioning to Churned
  if (toPhase === 'Churned') {
    if (assignments.churnReason) assignData.churnReason = assignments.churnReason
    if (assignments.churnNote  ) assignData.churnNote   = assignments.churnNote
  }
...
export async function seedMissingTrackers() {
  ...
  for (const account of accounts) {
    ...
    const hasCancelled = account.contracts.some(c => c.cancellationDate != null)
    const hasAny = account.contracts.length > 0
    // Active → AccountManagement | Expired (lapsed, no cancellation) → Expired | Churned (cancelled) → Churned | No contracts → DealClosure
    const phase = hasActive ? 'AccountManagement'
      : hasAny && !hasCancelled ? 'Expired'
      : hasAny ? 'Churned'
      : 'DealClosure'
...
// ─── Sync trackers to Expired / Churned based on contract state ──────────────
// Idempotent. Runs on every GET /api/onboarding so the kanban always matches
// the dashboard which uses contract state as source of truth.
//
//  • Churned  = at least one contract has cancellationDate set  (explicit cancel)
//  • Expired  = all contracts past endDate, none explicitly cancelled (lapse)
//
// Trackers already in the correct terminal phase are skipped.
export async function syncExpiredTrackers() {
  const now = new Date()

  // Fetch all trackers not yet in either terminal phase
  const trackers = await prisma.onboardingTracker.findMany({
    where: { phase: { notIn: ['Expired', 'Churned'] } },
    include: {
      account: {
        select: {
          contracts: { select: { cancellationDate: true, endDate: true } },
  ...
  for (const tracker of trackers) {
    const contracts = tracker.account?.contracts ?? []
    if (contracts.length === 0) continue

    const hasActive    = contracts.some(c => !c.cancellationDate && new Date(c.endDate) >= now)
    const hasCancelled = contracts.some(c => c.cancellationDate != null)

    let targetPhase = null
    if (!hasActive && hasCancelled) targetPhase = 'Churned'  // explicit cancellation
    else if (!hasActive && !hasCancelled) targetPhase = 'Expired' // natural lapse

    if (targetPhase) {
      await prisma.onboardingTracker.update({
        where: { id: tracker.id },
        data:  { phase: targetPhase, phaseStartedAt: now },
      })
      synced++
    }
  }
...
export async function addNote(trackerId, content, author, authorId) {
...
export async function assignOnboardingTeam(id, { onboardingSpecialistId, trainingSpecialistId }) {
  const data = {}
  if (onboardingSpecialistId !== undefined) data.onboardingSpecialistId = onboardingSpecialistId || null
  if (trainingSpecialistId   !== undefined) data.trainingSpecialistId   = trainingSpecialistId   || null
  return prisma.onboardingTracker.update({
    where: { id: Number(id) },
    data,
  })
