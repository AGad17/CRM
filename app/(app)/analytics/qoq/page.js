...
export default function QoQPage() {
  ...
  const { data = [], isLoading, isError, refetch } = useQuery({
    queryKey: ['analytics-qoq', country, leadSources],
    queryFn: () => {
      const p = new URLSearchParams()
      if (country) p.set('country', country)
      if (leadSources.length > 0) p.set('leadSources', leadSources.join(','))
      return fetch(`/api/analytics/qoq?${p}`).then((r) => {
        if (!r.ok) throw new Error('Failed to load quarter-over-quarter data')
        return r.json()
      })
    },
  })
    ...
    <div className="space-y-5">
      ...
      {chartData.length > 1 && (
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm px-5 pt-5 pb-4">
          <p className="text-[11px] font-bold text-gray-400 uppercase tracking-widest mb-4">Total MRR Signed by Quarter</p>
          <ResponsiveContainer width="100%" height={200}>
            <BarChart data={chartData} margin={{ top: 4, right: 16, bottom: 0, left: 8 }}>
              ...
              <Bar dataKey="totalMRRSigned" radius={[6, 6, 0, 0]} maxBarSize={40}>
                {chartData.map((_, i) => (
      ...
      <AnalyticsTable data={filtered} isLoading={isLoading} periodLabel="Quarter" exportFilename="qoq.csv" />
    </div>
  )
}
