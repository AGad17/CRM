...
// ─── Sync active trackers to Expired when contracts have lapsed ───────────────
// Idempotent: only moves trackers that are still in an active phase (not already
// Expired or Churned) when all of the account's contracts have expired without
// a cancellation date (natural lapse = Expired, not Churned).
export async function syncExpiredTrackers() {
  const now = new Date()

  // Check all non-Expired trackers (including Churned — some were incorrectly
  // placed in Churned without a cancellationDate; the contract state is the
  // source of truth).
  const trackers = await prisma.onboardingTracker.findMany({
    where: { phase: { not: 'Expired' } },
    include: {
      account: {
        select: {
          contracts: { select: { cancellationDate: true, endDate: true } },
  ...
  for (const tracker of trackers) {
    ...
    // Expired = all contracts past endDate, none explicitly cancelled
    if (!hasActive && !hasCancelled) {
      await prisma.onboardingTracker.update({
        where: { id: tracker.id },
        data:  { phase: 'Expired', phaseStartedAt: now },
      })
      synced++
    }
  }
